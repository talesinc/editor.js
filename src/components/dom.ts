...
export default class Dom {
  ...
  public static isEmpty(node: Node): boolean {
    ...
    while (treeWalker.length > 0) {
      ...
      // If node is marked with "no-block-content" class, do not traverse further
      const element = node as HTMLElement;
      if (element && element.classList?.contains("no-block-content")) {
        continue;
      }
  ...
  public static isExtensionNode(node: Node): boolean {
    const extensions = ["GRAMMARLY-EXTENSION"];
